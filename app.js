...
function updateUIBasedOnPlan() {
  ...
  // Optionally hide/show pro features – add class "pro-feature" to elements that should be hidden for free users
...
// Global showView function for links
window.showView = function(viewId) {
  navItems.forEach(n => n.classList.remove('active'));
  document.querySelector(`[data-view="${viewId}"]`).classList.add('active');
  views.forEach(v => v.classList.remove('active'));
  document.getElementById(`view-${viewId}`).classList.add('active');
  pageTitle.textContent = document.querySelector(`[data-view="${viewId}"] span`).textContent;
};
...
startBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(errData.error || 'Start failed');
    }
    streamActive = true;
    updateStreamUI(true);
    startTimer();
...
document.addEventListener('click', async (e) => {
  if (e.target.id === 'pro-plan' && e.target.textContent !== 'Current Plan') {
    try {
      ...
      const priceId = 'price_1R0GbpIXqpnCRRNU8G8M7E2S'; // <-- CHANGE THIS
